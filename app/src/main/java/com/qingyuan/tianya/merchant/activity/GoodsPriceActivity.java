...
public class GoodsPriceActivity extends BaseActivity{
    ...
    public void initView() {
        ...
        price = bundle.getString("goods_price");
        head = (HeaderView) findViewById(R.id.goods_price_head);
        ed = ((EditText) findViewById(R.id.goods_price_ed));
    }
    ...
    private void toUpdatePrice() {
        ...
        if (StringUtil.isNotEmpty(ed.getText().toString().trim())){
            params.put("price",ed.getText().toString().trim());
        }else {
            toast("商品价格不能为空");
            return;
        }
